...
export function getSupabaseConfig(override = {}) {
    const supabaseUrlRaw = override.supabaseUrl || readProcessEnv(SUPABASE_ENV_URL);
    const supabaseUrl =
        typeof supabaseUrlRaw === 'string' ? supabaseUrlRaw.trim().replace(/\/$/, '') : '';
    const serviceKey = (override.serviceKey || readProcessEnv(SUPABASE_ENV_SERVICE_ROLE_KEY)).trim();
    return { supabaseUrl, serviceKey };
}

/**
 * missing_supabase_config 원인 파악용 (키 값은 로그하지 않음)
...
export function logSupabaseEnvDiagnostics(context = '') {
    ...
    const diag = {
        ...
        readNames: {
            [SUPABASE_ENV_URL]: !!readProcessEnv(SUPABASE_ENV_URL),
            [SUPABASE_ENV_SERVICE_ROLE_KEY]: !!readProcessEnv(SUPABASE_ENV_SERVICE_ROLE_KEY),
        },
...
function assertSupabaseConfig(configOverride, context) {
...
export async function insertExperimentEvents(rows, configOverride) {
    ...
    const cfg = assertSupabaseConfig(configOverride, 'insertExperimentEvents');
    if (!cfg) {
...
export async function upsertParticipantSummary(summaryRow, configOverride) {
    ...
    const cfg = assertSupabaseConfig(configOverride, 'upsertParticipantSummary');
    if (!cfg) {
