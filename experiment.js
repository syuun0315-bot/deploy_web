...
const REVIEW_ACTIVE_QUESTION_HINT =
    "사실 3문제, 이해 3문제 이상 만들어주세요. 시험을 볼 준비가 되었다고 판단했을 때 '다음' 버튼을 눌러 다음 단계로 이동해주세요.";
...
function updateReviewStageBar() {
    ...
    hintEl.textContent = REVIEW_ACTIVE_QUESTION_HINT;
}
...
function setupSurvey() {
    ...
    surveyQuestions.forEach((q) => {
        ...
        if (q.type === 'scale') {
            ...
            // 숫자 문항(기존 9번)에는 '점' 단위를 표시
            const unit = q.number === 8 ? '점' : '';
            questionHTML += `
...
function setReviewInstructionCondition2Page4() {
    ...
    if (!content) return;
    content.innerHTML = `
        <p class="review-instruction-emphasis">${REVIEW_ACTIVE_QUESTION_HINT}</p>
